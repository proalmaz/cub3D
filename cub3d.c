...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/01 18:31:35 by vping             #+#    #+#             */
/*   Updated: 2021/03/02 21:49:48 by vping            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"
#define A s.sprite_screen_x
#define B s.sprite_width
#define C data.bits_per_pixel
#define D data.line_length
#define E data.endian
#define F data.img

static	void	malloc_arrays(t_all *cub)
{
...
static	void	print_sprite(t_all *cub)
{
	...
	while (++i < cub->p.coll_sprite)
	{
		...
		{
			s.tex_x = (int)(256 * (stripe - (-B / 2 + A)) * TEXWIDTH / B) / 256;
...
static	void	print_map(t_all *cub)
{
	...
	cub->data.addr = mlx_get_data_addr(cub->F, &cub->C, &cub->D, &cub->E);
	x = -1;
	while (++x < cub->p.res_w)
	{
